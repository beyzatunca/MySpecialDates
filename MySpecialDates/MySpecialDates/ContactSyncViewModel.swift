...
class ContactSyncViewModel: ObservableObject {
    ...
    func addManualBirthday(
        ...
        do {
            ...
            try await birthdayManager.addManualBirthday(birthdayEntry, for: userId)
    ...
    private func updateStatistics(userId: String) async {
        ...
        let totalBirthdays = birthdays.count
        let today = Date()
        let calendar = Calendar.current
        let thisMonthBirthdays = birthdays.filter { 
            calendar.component(.month, from: $0.birthday) == calendar.component(.month, from: today)
        }.count
        let upcomingBirthdays = birthdays.filter { $0.daysUntilBirthday <= 30 }.count
        
        statistics = BirthdayStatistics(
            totalBirthdays: totalBirthdays,
            upcomingBirthdays: upcomingBirthdays,
            thisMonthBirthdays: thisMonthBirthdays,
            fromContacts: birthdays.filter { $0.isFromContact }.count,
            manual: birthdays.filter { !$0.isFromContact }.count
        )
    }
