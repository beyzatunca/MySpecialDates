...
protocol BirthdayManagerProtocol {
    ...
    func addManualBirthday(_ birthday: BirthdayEntry, for userId: String) async throws
}
...
class MockFirebaseAuthService: FirebaseAuthServiceProtocol {
    nonisolated(unsafe) private var mockCurrentUser: FirebaseUserModel?
    ...
    nonisolated func getCurrentUser() -> FirebaseUserModel? {
...
class MockBirthdayManager: BirthdayManagerProtocol {
    nonisolated private let firebaseRepository: FirebaseUserRepositoryProtocol
    
    nonisolated init(firebaseRepository: FirebaseUserRepositoryProtocol = MockFirebaseUserRepository()) {
    ...
    func addManualBirthday(_ birthday: BirthdayEntry, for userId: String) async throws {
        ...
        print("Mock: Adding manual birthday for \(birthday.contactName)")
    }
}
